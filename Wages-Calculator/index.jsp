					...
					<tbody>
						...
						<%
							...
							try {
								...
								rs1 = pstmt.executeQuery(); // 쿼리를 실행하고 결과를 ResultSet 객체에 담는다.
				...
				<tfoot>
					<%
						...
						try {
							...
							String sql = "select COUNT(ID) as sumDay, Concat(SEC_TO_TIME(SUM(TIME_TO_SEC(totalTime)))) as sumTime, SUM(dailywages) as sumPay FROM JUNE";
							ps1 = connnection.prepareStatement(sql); // prepareStatement에서 해당 sql을 미리 컴파일한다.
							rs2 = ps1.executeQuery();

							while (rs2.next()) {

								int sumDay = rs2.getInt("sumDay");

								String sumTime = rs2.getString("sumTime");

								int sumPay = rs2.getInt("sumPay");
					%>
					<tr>
