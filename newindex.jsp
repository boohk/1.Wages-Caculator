				...
				<table cellpadding="0" cellspacing="0" border="0">
					<tbody>
						...
						<%
								...
								ps2 = conn.prepareStatement(sql1); // prepareStatement에서 해당 sql을 미리 컴파일한다.
								rs2 = ps2.executeQuery();
								rs2.first();
								total[0] = rs2.getString("sumday");

								String sql2 = "SELECT Concat(\"\",SEC_TO_TIME(SUM(TIME_TO_SEC(totalTime)))) as sumTime from june";
								ps2 = conn.prepareStatement(sql2); // prepareStatement에서 해당 sql을 미리 컴파일한다.
								rs2 = ps2.executeQuery();
								...
								ps2 = conn.prepareStatement(sql3); // prepareStatement에서 해당 sql을 미리 컴파일한다.
								rs2 = ps2.executeQuery();
